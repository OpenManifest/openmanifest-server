...
RSpec.describe Federations::ApfSync do
  describe "syncing federation qualifications" do
    ...
    let!(:user_federation) { UserFederation.find_by(user: user, federation: Federation.find_by(slug: :apf)) || create(:user_federation, federation: Federation.find_by(slug: :apf), user: user, uid: "21922484") }
    let!(:apf_response) do
