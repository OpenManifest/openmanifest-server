...
RSpec.describe Federations::ApfSync do
  describe "syncing federation qualifications" do
    ...
    let!(:dropzone_user) { create(:dropzone_user, dropzone: dropzone, user: user, license: nil) }
    let!(:user_federation) do
      UserFederation.find_by(user: user,
                             federation: Federation.find_by(slug: :apf)) || create(:user_federation, federation: Federation.find_by(slug: :apf), user: user,
                                                                                                     uid: "21922484")
    end
    let!(:apf_response) do
    ...
    let!(:access_context) { ApplicationInteraction::AccessContext.new(dropzone_user) }
    ...
    it {
      expect(dropzone_user.reload.license.id).to eq user.reload.licenses.includes(:federation).find_by(federation: { slug: :apf }).id
    }
  end
